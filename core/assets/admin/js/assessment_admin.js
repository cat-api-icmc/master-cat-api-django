const CRITERIA_LABELS = {
    ...
    'Trule': 'TRULE - Traço Máximo (potencialmente ponderado) da Matriz de Informação',
    'Arule': 'ARULE - Traço Mínimo (potencialmente ponderado) da Matriz de Covariância Assintótica',
...
const get_criterias = (model) => {
    ...
    if (IRT_MODELS.includes(model)) {
        return base_criterias.concat(['MI', 'MEPV', 'MLWI', 'MPWI', 'MEI', 'KL', 'KLn', 'IKLP', 'IKL', 'IKLn', 'IKLPn']);
    }
...
const populate_criteria_options = (model) => {
    ...
    criteria_select.value = criterias.includes(previousValue) ? previousValue : criterias[0];
};
